    ...
    public class GXDLMSReader
    {
        ...
        private void Client_OnKeys(object sender, GXCryptoKeyParameter args)
        {
            try
            {
                if (args.Encrypt)
                {
                    ...
                    //MessageBox.ShowAsync("Client private key:" + args.PrivateKey.ToHex());
                }
                else
                {
                    //Find public key.
                    if (args.SystemTitle == null)
                    {
                        ...
                        //MessageBox.ShowAsync("Server public key:" + pk.SerialNumber);
                    }
                    else
                    {
                        string path = GetPath(args.SecuritySuite, args.CertificateType, "Certificates", args.SystemTitle);
        ...
        public void SNRMRequest()
        {
            ...
            if (data != null)
            {
                ...
                {
                    MessageBox.ShowAsync("Send SNRM request." + GXCommon.ToHex(data, true));
                }
                ReadDataBlock(data, reply);
                if (Trace == TraceLevel.Verbose)
                {
                    MessageBox.ShowAsync("Parsing UA reply." + reply.ToString());
                }
                //Has server accepted client.
                Client.ParseUAResponse(reply.Data);
                if (Trace > TraceLevel.Info)
                {
                    MessageBox.ShowAsync("Parsing UA reply succeeded.");
                }
            }
        }
        ...
        public void AarqRequest()
        {
            ...
            foreach (byte[] it in Client.AARQRequest())
            {
                if (Trace > TraceLevel.Info)
                {
                    MessageBox.ShowAsync("Send AARQ request", GXCommon.ToHex(it, true));
                }
                reply.Clear();
            ...
            if (Trace > TraceLevel.Info)
            {
                MessageBox.ShowAsync("Parsing AARE reply" + reply.ToString());
            }
            //Parse reply.
            ...
            if (Trace > TraceLevel.Info)
            {
                MessageBox.ShowAsync("Parsing AARE reply succeeded.");
            }
        }
            ...
            if (!string.IsNullOrEmpty(InvocationCounter) && Client.Ciphering != null && Client.Ciphering.Security != Security.None)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        MessageBox.ShowAsync("Invocation counter: " + Convert.ToString(Client.Ciphering.InvocationCounter));
                        reply.Clear();
                        Disconnect();
                    }
                    catch (Exception Ex)
        ...
        void InitializeOpticalHead()
        {
            ...
            if (Trace > TraceLevel.Info)
            {
                MessageBox.ShowAsync("IEC Sending:" + data);
            }
            ReceiveParameters<string> p = new ReceiveParameters<string>()
            {
                AllData = false,
            ...
            if (Trace > TraceLevel.Info)
            {
                MessageBox.ShowAsync("IEC received: " + p.Reply);
            }
            int pos = 0;
            ...
            if (Trace > TraceLevel.Info)
            {
                MessageBox.ShowAsync(DateTime.Now.ToLongTimeString() + "\tBaudRate is : " + BaudRate.ToString());
            }
            //Send ACK
            ...
            if (Trace > TraceLevel.Info)
            {
                //MessageBox.ShowAsync("DateTime.Now.ToLongTimeString() + "\tMoving to mode E.", arr");
            }
            lock (Media.Synchronous)
            {
                p.Reply = null;
                ...
                if (p.Reply != null)
                {
                    if (Trace > TraceLevel.Info)
                    {
                        MessageBox.ShowAsync("Received: " + p.Reply);
                    }
                }
                Media.Close();
        ...
        {
            MessageBox.ShowAsync("Standard: " + Client.Standard);
            if (Client.Ciphering.Security != Security.None)
            {
                MessageBox.ShowAsync("Security: " + Client.Ciphering.Security);
                MessageBox.ShowAsync("System title: " + GXCommon.ToHex(Client.Ciphering.SystemTitle, true));
                MessageBox.ShowAsync("Authentication key: " + GXCommon.ToHex(Client.Ciphering.AuthenticationKey, true));
                MessageBox.ShowAsync("Block cipher key " + GXCommon.ToHex(Client.Ciphering.BlockCipherKey, true));
                if (Client.Ciphering.DedicatedKey != null)
                {
                    MessageBox.ShowAsync("Dedicated key: " + GXCommon.ToHex(Client.Ciphering.DedicatedKey, true));
                }
            }
            UpdateFrameCounter();
            InitializeOpticalHead();
            ...
            //Parse reply.
            Client.ParseAAREResponse(reply.Data);
            MessageBox.ShowAsync("Conformance: " + Client.NegotiatedConformance);
            reply.Clear();
            //Get challenge Is HLS authentication is used.
        ...
        public void GetScalersAndUnits()
        {
            ...
            {
                MainWindowViewModel.connectInfoLog += "Read scalers and units from the device.";
                //MessageBox.ShowAsync("Read scalers and units from the device.");
            }
            ...
            if ((Client.NegotiatedConformance & Gurux.DLMS.Enums.Conformance.MultipleReferences) == 0)
            {
                ...
                foreach (GXDLMSObject it in objs)
                {
                    try
                    {
                        ...
                        {
                            MainWindowViewModel.connectInfoLog += $"{it.Name.ToString()}";
                           //MessageBox.ShowAsync(it.Name.ToString());
                            Read(it, 3);
                        }
                        if (it is GXDLMSDemandRegister && Client.CanRead(it, 4))
                        {
        ...
        public void GetProfileGenericColumns()
        {
            ...
            foreach (GXDLMSObject it in Client.Objects.GetObjects(ObjectType.ProfileGeneric))
            {
                try
                {
                    ...
                    if (Trace > TraceLevel.Warning)
                    {
                        MainWindowViewModel.connectInfoLog += $"{it.LogicalName}";
                        //MessageBox.ShowAsync(it.LogicalName);
                    }
                    Read(it, 3);
                    //If info.
                    if (Trace > TraceLevel.Warning)
                    {
                        ...
                        MainWindowViewModel.connectInfoLog += $"{sb.ToString()}";
                       //MessageBox.ShowAsync(sb.ToString());
                    }
                }
                catch (Exception)
                {
        ...
        public void ShowValue(object val, int pos)
        {
            ...
            if (Trace > TraceLevel.Warning)
            {
                ...
                MainWindowViewModel.connectInfoLog += $"Index: {pos}" + $" Value: {val} ";
                //MessageBox.ShowAsync("Index: " + pos + " Value: " + val);
            }
        }
        ...
        public void GetProfileGenerics()
        {
            ...
            foreach (GXDLMSObject it in Client.Objects.GetObjects(ObjectType.ProfileGeneric))
            {
                ...
                if (Trace > TraceLevel.Warning)
                {
                    //MessageBox.ShowAsync("-------- Reading " + it.GetType().Name + " " + it.Name + " " + it.Description);
                    MainWindowViewModel.connectInfoLog += "-------- Reading " + $"{it.GetType().Name}" + " " + $"{it.Name}" + " " + $"{it.Description}";
                }
                long entriesInUse = -1;
                ...
                if (Trace > TraceLevel.Warning)
                {
                    MessageBox.ShowAsync("Entries: " + entriesInUse + "/" + entries);
                }
                ...
                if ((Client.NegotiatedConformance & (Gurux.DLMS.Enums.Conformance.ParameterizedAccess | Gurux.DLMS.Enums.Conformance.SelectiveAccess)) != 0)
                {
                    ...
                    {
                        MessageBox.ShowAsync("Error! Failed to read first row: " + ex.Message);
                        //Continue reading.
                    }
                }
                //All meters are not supporting parameterized read.
                if ((Client.NegotiatedConformance & (Gurux.DLMS.Enums.Conformance.ParameterizedAccess | Gurux.DLMS.Enums.Conformance.SelectiveAccess)) != 0)
                {
                    ...
                    {
                        MessageBox.ShowAsync("Error! Failed to read last day: " + ex.Message);
                        //Continue reading.
                    }
                }
            }
        ...
        /// </remarks>
        public void GetReadOut() //чтение всех данных со счетчика
        {
            //var customGXDLMSObject = new GXDLMSObject{0.0.40.0.3.255 Ch. 0 Association  #3 None}
            foreach (GXDLMSObject it in Client.Objects)
            {
                ...
                //дальнейшие действия выполним только для "0.0.96.1.3.255"
                if (it.LogicalName != "0.0.96.1.3.255")
                    continue;
                ...
                if (!(it is IGXDLMSBase))
                {
                    ...
                    {
                        MessageBox.ShowAsync("Unknown Interface: " + it.ObjectType.ToString());
                    }
                    continue;
                }
                if (Trace > TraceLevel.Warning)//сюда заходит
                {
                    MainWindowViewModel.connectInfoLog += "-------- Reading " + $"{it.GetType().Name}" + $"{it.Name}" + $"{it.Description}";
                    //MessageBox.ShowAsync("-------- Reading " + it.GetType().Name + " " + it.Name + " " + it.Description);
                }
                foreach (int pos in (it as IGXDLMSBase).GetAttributeIndexToRead(true))
                {
                    ...
                    {
                        MessageBox.ShowAsync("Error! " + it.GetType().Name + " " + it.Name + "Index: " + pos + " " + ex.Message);
                        ...
                        if (notify.IsComplete && notify.Data.Data != null)
                        {
                            ...
                            if (!notify.IsMoreData)
                            {
                                if (notify.PrimeDc != null)
                                {
                                    ...
                                    MessageBox.ShowAsync(notify.PrimeDc.ToString());
                                }
                                else
                                {
        ...
        public object Read(GXDLMSObject it, int attributeIndex)
        {
            ...
            {
                MessageBox.ShowAsync("Can't read " + it.ToString() + ". Not enought acccess rights.");
            }
            return null;
        }
        ...
        public void Disconnect()
        {
            if (Media != null && Client != null)
            {
                try
                {
                    if (Trace > TraceLevel.Info)
                    {
                        MessageBox.ShowAsync("Disconnecting from the meter.");
                    }
                    GXReplyData reply = new GXReplyData();
        ...
        public void Release()
        {
            if (Media != null && Client != null)
            {
                try
                {
                    if (Trace > TraceLevel.Info)
                    {
                        MessageBox.ShowAsync("Release from the meter.");
                    }
                    GXReplyData reply = new GXReplyData();
                ...
                catch (Exception ex)
                {
                    ...
                    MessageBox.ShowAsync("Release failed. " + ex.Message);
                }
            }
        }
